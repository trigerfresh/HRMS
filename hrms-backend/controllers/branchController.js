...
exports.createBranch = async (req, res) => {
  try {
    ...
    const result = await sql.query`
      INSERT INTO branch (
        branch_name, branch_code, company_name, company_id, email, address, city, pincode,
        costing_method, def_purchase_ac, def_sales_ac, def_branch_recv_ac, def_branch_desp_ac,
        phone, active, created_by, created_on
      )
      OUTPUT INSERTED.*
      VALUES (
        ${data.branch_name}, ${data.branch_code}, ${data.company_name}, ${data.company_id}, ${data.email}, ${data.address}, ${data.city}, ${data.pincode},
        ${data.costing_method}, ${data.def_purchase_ac}, ${data.def_sales_ac}, ${data.def_branch_recv_ac}, ${data.def_branch_desp_ac},
        ${data.phone}, 0, ${req.user.id}, GETDATE()
      )
    `
...
exports.getBranches = async (req, res) => {
  try {
    let query = `SELECT * FROM branch WHERE active = 0 ORDER BY created_on DESC`
...
exports.updateBranch = async (req, res) => {
  try {
    ...
    if (data.email) {
      const checkEmail = await sql.query`
        SELECT TOP 1 * FROM branch
        WHERE email = ${data.email} AND id != ${id} AND active = 0
      `
      if (checkEmail.recordset.length > 0) {
        return res.status(400).json({ message: 'Email ID already exists' })
      }
...
exports.deleteBranch = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Error deactivating branch', error: error.message })
  }
}
...
exports.exportBranchesToExcel = async (req, res) => {
  try {
    const result = await sql.query(
      `SELECT * FROM branch WHERE active = 0 ORDER BY created_on DESC`,
    )
...
exports.getBranchesByCompany = async (req, res) => {
  try {
    ...
    const result = await sql.query`
      SELECT * FROM branch WHERE company_id=${companyId} AND active=0
    `
    res.json(result.recordset)
  } catch (err) {
    console.error(err)
