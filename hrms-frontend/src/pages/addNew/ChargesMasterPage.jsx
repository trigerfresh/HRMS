...
export default function ChargesMasterPage() {
  ...
  const searchOptions = [
    ...
    { value: "labelToDisplay", label: "Label to display" },
  ];
  ...
  const handleReset = () => {
    setSearchFields([{ field: "chargesType", keyword: "" }]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.chargesType.trim()) {
      alert("Please fix the validation error.");
      setValidationErrors((prev) => ({
        ...prev, // keep existing errors
        chargesType: "Charges type is required", // add/update this field's error
      }));
      return;
    }
    try {
      if (editingCharge) {
        await axios.put(
          `${API_BASE_URL}/${editingCharge._id}`,
          formData,
          getAuthHeaders(),
        );
  ...
  const handleEdit = (charge) => {
    ...
    setFormData({
      ...
      labelToDisplay: charge.labelToDisplay || "",
    });
      ...
      {showForm && (
        <Card>
          <h2 className="card-header mb-4">
            {editingCharge ? (
              <span>Edit Charge - {editingCharge.chargesType}</span>
            ) : (
              "Add New Charge"
            )}
          </h2>
          ...
          <Form onSubmit={handleSubmit} className="charges-master-form">
            <Row>
              <Col md={6} className="mb-3">
                <Form.Group controlId="chargesType">
                  ...
                  <Form.Control
                    ...
                    isInvalid={!!validationErrors.chargesType}
                  />
                  <Form.Control.Feedback type="invalid">
                    {validationErrors.chargesType}
                  </Form.Control.Feedback>
                </Form.Group>
              </Col>

              <Col md={6} className="mb-3">
                <Form.Group controlId="labelToDisplay">
                  <Form.Label>Label to display</Form.Label>
                  <Form.Control
                    name="labelToDisplay"
                    value={formData.labelToDisplay}
                    onChange={handleChange}
                ...
                {charges.length > 0 ? (
                  charges.map((charge, index) => (
                    <tr key={charge._id}>
                      ...
                      <td>{charge.labelToDisplay || ""}</td>
                      <td>
                        {charge.created_by ? charge.created_by.name : ""}
                        <br />
                        {charge.created_on &&
                          new Date(charge.created_on).toLocaleDateString()}
                      </td>
                      <td>
