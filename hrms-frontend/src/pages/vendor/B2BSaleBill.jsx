...
const B2BSaleBill = () => {
  ...
  const fetchSaleBills = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/b2b/sales-bill`, {
        params,
        ...getAuthHeaders(),
      });
      ...
      } else setError("Failed to load sale bills.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchInvoiceNo = async () => {
    try {
      const res = await axios.get(
        `${API_BASE_URL}/b2b/sales-bill/fetchInvoiceNo`,
        {
          ...getAuthHeaders(),
        },
      );
  ...
  const handleEdit = (bill) => {
    ...
    setFormData({
      ...
      cashDiscount: bill.cashDiscount || "",
      receivedAmt: bill.receivedAmt || "",
      receivedDate: formatDateForInput(bill.receivedDate) || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(
          `${API_BASE_URL}/b2b/sales-bill/${isEditing._id}`,
          data,
          config,
        );
        alert("Sale Bill updated successfully!");
        navigate(`/b2b/view-sale-bill?saleBillId=${isEditing._id}`, {
          state: { autoPrint: true },
        });
      } else {
        const res = await axios.post(
          `${API_BASE_URL}/b2b/sales-bill/`,
          data,
          config,
        );
        // console.log(res.data);
        navigate(`/b2b/view-sale-bill?saleBillId=${res.data.data}`, {
          state: { autoPrint: true },
        });
  ...
  const handleReceiveSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/b2b/sales-bill/receiveAmt`,
        {
          billId: recieveAmtData?._id,
          paidAmount: Number(receiveForm.amount),
          receivedDate: new Date(receiveForm.receivedDate),
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this sale bill?")) {
      try {
        await axios.delete(
          `${API_BASE_URL}/b2b/sales-bill/${id}`,
          getAuthHeaders(),
        );
        alert("Sale bill removed!");
        fetchSaleBills();
      } catch (err) {
        if (err.response?.status === 401) {
          localStorage.removeItem("token");
          window.location.href = "/login";
        } else alert("Sale bill remove failed!");
      }
    }
  };
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/b2b/sales-bill/export`,
        {
          params,
      ...
      link.download = `SaleBills_${randomNumber}.xlsx`;
      link.click();
    } catch (error) {
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h1 className="page-title">B2B Sale Bill</h1>
      ...
      {showForm && (
        <Card>
          <h2 className="card-header mb-4">
            {isEditing ? (
              <span>Edit Sale Bill - {isEditing.invoiceNo}</span>
            ) : (
              "Add Sale Bill"
            )}
          </h2>
                  ...
                  <td>
                    <Form.Group>
                      <Form.Control
                        ...
                        onChange={handleChange}
                        isInvalid={!!validationErrors.receivedAmt}
                      />
                      <Form.Control.Feedback type="invalid">
                        {validationErrors.receivedAmt}
                      </Form.Control.Feedback>
                    </Form.Group>
                  </td>
                </tr>
                ...
                {saleBills.length === 0 ? (
                  ...
                  saleBills.map((b) => (
                    <tr key={b._id}>
                      ...
                      <td>{b.receivedAmt || 0}</td>
                      <td>{b.totalGSTAmount || 0}</td>
                      <td>{b.totalPayableAmount - b.receivedAmt || 0}</td>
                      <td>
                      ...
                      <td>
                        <Dropdown align="end">
                          ...
                          <Dropdown.Menu
                            ...
                            renderOnMount
                          >
                            <DropdownItem onClick={() => handleReceiveAmt(b)}>
                              Receive Amt
                            </DropdownItem>
                            <DropdownItem
                              onClick={() =>
                                navigate(
                                  `/b2b/view-sale-bill?saleBillId=${b._id}`,
                                  {
                                    state: { autoPrint: true },
                                  },
                                )
              ...
              <Col md={12} className="mb-3">
                <Form.Group>
                  ...
                  <Form.Control
                    ...
                    onChange={handleReceiveChange}
                    isInvalid={!!RAvalidationErrors.receivedDate}
                  />
                  <Form.Control.Feedback type="invalid">
                    {RAvalidationErrors.receivedDate}
                  </Form.Control.Feedback>
                </Form.Group>
              </Col>
            ...
            <div className="d-flex flex-column">
              <b>
                <span className="text-secondary">
                  Bill amount: Rs {recieveAmtData.totalPayableAmount}
                </span>
              </b>
              <b>
                <span className="text-danger">
                  Balance amount: Rs{" "}
                  {recieveAmtData.totalPayableAmount -
                    recieveAmtData.receivedAmt}
                </span>
              </b>
            </div>
            <div>
