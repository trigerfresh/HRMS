...
const B2BCreditNote = () => {
  ...
  const fetchCreditNotes = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/b2b/credit-note`, {
        params,
        ...getAuthHeaders(),
      });
      ...
      } else setError("Failed to load credit notes.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchInvoiceNo = async () => {
    try {
      const res = await axios.get(
        `${API_BASE_URL}/b2b/credit-note/fetchInvoiceNo`,
        {
          ...getAuthHeaders(),
        },
      );
  ...
  const handleEdit = (note) => {
    ...
    setFormData({
      ...
      creditDate: note.creditDate ? formatDateForInput(note.creditDate) : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(
          `${API_BASE_URL}/b2b/credit-note/${isEditing._id}`,
          data,
          config,
        );
        alert("Credit Note updated successfully!");
        navigate(`/b2b/view-credit-note?creditNoteId=${isEditing._id}`, {
          state: { autoPrint: true },
        });
      } else {
        const res = await axios.post(
          `${API_BASE_URL}/b2b/credit-note/`,
          data,
          config,
        );
        // console.log(res.data);
        alert("Credit Note saved successfully!");
        navigate(`/b2b/view-credit-note?creditNoteId=${res.data.data}`, {
          state: { autoPrint: true },
        });
      }
      resetForm();
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this credit note?")) {
      try {
        await axios.delete(
          `${API_BASE_URL}/b2b/credit-note/${id}`,
          getAuthHeaders(),
        );
        alert("Credit note removed!");
        fetchCreditNotes();
      } catch (err) {
        if (err.response?.status === 401) {
          localStorage.removeItem("token");
          window.location.href = "/login";
        } else alert("Credit note remove failed!");
      }
    }
  };
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/b2b/credit-note/export`,
        {
          params,
      ...
      link.download = `CreditNote_${randomNumber}.xlsx`;
      link.click();
    } catch (error) {
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h1 className="page-title">B2B Credit Note</h1>
      ...
      {showForm && (
        <Card>
          <h2 className="card-header mb-4">
            {isEditing ? (
              <span>Edit Credit Note - {isEditing.invoiceNo}</span>
            ) : (
              "Add Credit Note"
            )}
          </h2>
                        ...
                        <Dropdown align="end">
                          ...
                          >
                            <DropdownItem
                              onClick={() =>
                                navigate(
                                  `/b2b/view-credit-note?creditNoteId=${n._id}`,
                                  {
                                    state: { autoPrint: true },
                                  },
                                )
