...
const CreateNewBill = ({ onCancel, isEditing, onSuccess }) => {
  ...
  const [rows, setRows] = useState([
    { type: "", sacCode: "", totalNos: "", costPerHead: "", total: "" },
  ]);
  ...
  const handleEditBill = async () => {
    ...
    setRows(
      ordersData.map((row) => ({
        ...
        type: row.type || "",
        sacCode: row.sacCode || "",
        totalNos: row.totalNos || 0,
        costPerHead: row.costPerHead || 0,
        total: row.total || 0,
      })),
    );
  };
  ...
  const handleChange = (index, field, value) => {
    ...
    const qty = Number(updated[index].totalNos);
    const rate = Number(updated[index].costPerHead);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(
          `${API_BASE_URL}/update-bill?invoiceNo=${isEditing}`,
          payload,
          getAuthHeaders(),
        );
    ...
    <Card>
      <div className="form-header">
        <h2 className="card-header mb-4">
          {isEditing ? <span>Edit Bill - {isEditing}</span> : "Create New Bill"}
        </h2>
      </div>
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <Form.Control
                        ...
                        value={row.totalNos}
                        onChange={(e) =>
                          handleChange(i, "totalNos", e.target.value)
                        }
                      />
                    </td>
                    <td>
                      <Form.Control
                        type="number"
                        name="costPerHead"
                        className="costPerHead"
                        value={row.costPerHead}
                        onChange={(e) =>
                          handleChange(i, "costPerHead", e.target.value)
                        }
                      />
                    </td>
                    <td>
